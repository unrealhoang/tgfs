...
fn from_row() {
    ...
    assert_eq!(todo.author.name, "john");
    assert_eq!(todo.author.role.as_ref().unwrap().id, role_id);
    assert_eq!(todo.author.role.as_ref().unwrap().kind, "admin");
    assert_eq!(todo.editor.name, "jack");
    assert!(todo.editor.role.is_none());
...
#[test]
fn std_types_from_row() {
    let connection = Connection::open_in_memory().unwrap();

    let has_permission = connection
        .query_row("SELECT EXISTS (SELECT 1)", [], bool::try_from_row)
        .unwrap();
    let count = connection
        .query_row("SELECT 42", [], i64::try_from_row)
        .unwrap();
    let label = connection
        .query_row("SELECT 'mapped'", [], String::try_from_row)
        .unwrap();
    let bytes = connection
        .query_row("SELECT X'010203'", [], <[u8; 3]>::try_from_row)
        .unwrap();
    let missing = connection
        .query_row("SELECT NULL", [], Option::<i64>::try_from_row)
        .unwrap();

    assert!(has_permission);
    assert_eq!(count, 42);
    assert_eq!(label, "mapped");
