...
The generated unprefixed mapper uses static column names directly, without allocating prefixed
column-name strings. Prefix strings are only constructed when a non-empty runtime prefix is used.

### Standard Types

Common standard-library types can map the first result column directly. This includes booleans,
integer and floating-point types, strings, byte vectors and arrays, reference-counted strings, and
