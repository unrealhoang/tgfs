//! High-level operations backing the `tgfs` CLI subcommands.
//!
//! These functions are the application layer over the library modules. They
//! are reused by the binary in `src/main.rs`, and are available to other
//! crates, but they are intentionally coupled to interactive/CLI concerns
//! (global config on disk, stdout messages) rather than being a minimal core
//! API.
...
use crate::config::{GlobalConfig, Repo, RepoConfig};
use crate::context::RepoContext;
use crate::index::Index;
use crate::key::{configure_repo_encryption, generate_keyfile};
use crate::tg::Tg;
use crate::{config, key, restore, snapshot, sync};
...
async fn open_repo() -> Result<RepoContext> {
    let repo = Repo::discover(&std::env::current_dir()?)?;
    let index = Index::open(&repo.index_path())?;
    let global = GlobalConfig::load()?;
    let tg = connect_authorized(&global).await?;
    Ok(RepoContext::new(tg, index, repo))
}
